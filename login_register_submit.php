...
if($type=='register'){
    ...
    if($check>0){
        ...
        'msg'=>'Email id already registered','field'=>'email_error');
        
    }else{
        $new_password=password_hash($password,PASSWORD_BCRYPT);
        $rand_str=rand_str();
        mysqli_query($con,"insert into user(name,email,mobile,password,status,email_verify,rand_str,added_on) 
        values('$name','$email','$mobile','$new_password','1','0','$rand_str','$added_on')");
        
        $id=mysqli_insert_id($con);
        $html=FRONT_SITE_PATH."verify?id=".$rand_str;
        send_email($email,$html,'Verify your email');

        $arr=array('status'=>'success',
		'msg'=>'Please check your email to verify your account','field'=>'form_msg');
    }
...
if($type=='login'){
    ...
    if($check>0){
        ...
        if($email_verify==1){
            ...
            $arr=array('status'=>'error','msg'=>'Please varify your email');
        }
	...
	if($check>0){	
		...
		if($email_verify==1){
			if($status==1){
				...
				$arr=array('status'=>'success','msg'=>'Password has been reset and send it to your email');

...
function send_email($email,$html,$subject){
	...
	$mail->SMTPSecure="tls";
	$mail->SMTPAuth=true;
	$mail->Username="srawal592@gmail.com";
	$mail->Password="9868784710";
	$mail->SetFrom("srawal592@gmail.com");
	...
	function manageUserCart($uid,$qty,$attr){
		global $con;
		$res=mysqli_query($con,"select * from dish_cart where user_id='$uid'
        and dish_detail_id='$attr'");
        if(mysqli_num_rows($res)>0){
            ...
            mysqli_query($con,"insert into dish_cart(user_id,dish_detail_id,qty,added_on) 
            values('$uid','$attr','$qty','$added_on')");
        }	
	}
function getDishCartStatus(){
		...
		foreach($dishDetailsID as $id){
			$res=mysqli_query($con,"select dish_details.status,
			dish.status as dish_status,dish.id 
			from dish_details,dish where dish_details.id='$id' 
			and dish_details.dish_id=dish.id");
			$row=mysqli_fetch_assoc($res);
...
function getUserFullCart($attr_id=''){
	...
	function getDishDetailById($id){
		global $con;
		$res=mysqli_query($con,"select dish.dish,dish.image,dish_details.price from dish_details,dish where dish_details.id='$id' 
        and dish.id=dish_details.dish_id"); 
	  	$row=mysqli_fetch_assoc($res);
	    return $row;
...
function getOrderDetails($oid){
		global $con;
		$sql="select order_details.price,order_details.qty,dish_details.attribute,dish.dish,dish.image,order_details.dish_details_id
		from order_details,dish_details,dish
		WHERE
		order_details.order_id=$oid AND
	...
	function getDeliveryBoyNameById($id){
		global $con;
		$sql="select name,mobile from delivery_person where id='$id'";
		$data=array();
		$res=mysqli_query($con,$sql);
		if(mysqli_num_rows($res)>0){
			$row=mysqli_fetch_assoc($res);
	...
	function getRating($did,$oid){
		global $con;
		$sql="select * from rating where order_id='$oid' and dish_details_id='$did'";
		$res=mysqli_query($con,$sql);
		if(mysqli_num_rows($res)>0){
			$row=mysqli_fetch_assoc($res);
			$rating=$row['rating'];
	...
	function getRatingByDishId($id){
		...
		if($row['total']>0){
			...
			echo "<span class='rating'> ($totalRate)</span>";
		}
	}
	function getSale($start,$end){
		global $con;
		$sql="select sum(total_price) as total_price from 
		order_master where added_on between '$start' and '$end' 
		and order_status=4";
		$res=mysqli_query($con,$sql);
		
		while($row=mysqli_fetch_assoc($res)){
			return 'Rs.'.$row['total_price'];
		}
	}
			  ...
			  <table class="email-content" width="100%" cellpadding="0" cellspacing="0" role="presentation">
				<tr>
				  <td class="email-masthead">
					...
					<img src="https://i.ibb.co/BjLmwS0/logo1-1.png"/>
				  </a>
				  </td>
				</tr>
				<!-- Email Body -->
				<tr>
					  ...
					  <tr>
						<td class="content-cell">
						  <div class="f-fallback">
							<h1>Hi, '.$name.'</h1>
							<p>Thank your for placing your order.</p>
				  ...
				  <strong>Order ID:</strong> '.$order_id.'
								  ...
								  <table class="purchase_content" width="100%" cellpadding="0" cellspacing="0">
									<tr>
									  ...
									  <th class="purchase_heading" align="right">
										<p class="f-fallback">Amount(Rs)</p>
									  </th>
									</tr>';
									$total_price=0;
									...
									$html.='<tr>
									  ...
									  <td width="20%" class="purchase_footer" valign="middle">
										<p class="f-fallback purchase_total">Rs.'.$total_price.'</p>
									  </td>
									</tr>
								  </table>
								</td>
							...
							<p>If you have any questions about our service, please <a href="'.FRONT_SITE_PATH.'contact-us">Contact Us</a> for help.</p>
							<p>
